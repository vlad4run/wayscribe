...
├── wayscribe/
│   ├── __main__.py             # CLI: daemon|toggle|status|stop|cancel|doctor|oneshot|lang|version|log
...
- **No local model.** The repo only HTTP-POSTs WAV bytes to the STT backend. All
  model work (NPU/Whisper) lives there; the reference backend is the sibling
  `../fastflowlm-docker/`.
